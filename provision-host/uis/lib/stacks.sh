...
_STACK_DATA=(
    ...
    "ai-local|Local AI Stack|Run AI models locally with a unified API and chat interface|AI|ai,llm,ollama,openai,chat,local|Self-hosted AI with Ollama, LiteLLM, and Open WebUI|ollama,litellm,openwebui|litellm|The local AI stack enables running large language models on your own infrastructure. Ollama runs the models, LiteLLM provides a unified OpenAI-compatible API, and Open WebUI offers a ChatGPT-like interface for users.|/docs/stacks/ai-local|ai-local-stack-logo.svg"
