...
# File: provision-host/kubernetes/10-data-science/not-in-use/02-remove-data-science.sh
# Description: Remove Databricks Replacement Data Science stack from Kubernetes cluster
...
PLAYBOOK_PATH="$ANSIBLE_DIR/playbooks/300-remove-data-science.yml"

# Default target host
TARGET_HOST="${1:-rancher-desktop}"

# Display script purpose
echo "🗑️  $(basename "$0"): Removing Databricks Replacement Data Science stack from Kubernetes cluster..."
...
run_playbook() {
    echo "🔧 $(basename "$0"): Running Ansible playbook for Data Science removal..."
...
if run_playbook; then
    echo
    echo "✅ $(basename "$0"): Data Science stack removal completed successfully!"
    echo "🎯 Target: $TARGET_HOST"
    echo "📝 Note: Spark Operator and JupyterHub have been removed"
    echo "🔐 Note: urbalurba-secrets preserved for future deployments"
    echo "📁 Note: Namespaces spark-operator and jupyterhub removed"
    echo "♻️  Ready for fresh installation: ./02-setup-data-science.sh"
else
    echo
    echo "❌ $(basename "$0"): Data Science stack removal failed!"
