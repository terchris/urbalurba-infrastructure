#!/bin/bash
# filename: provision-host/kubernetes/01-default-apps/08-setup-rabbitmq.sh
# description: Setup RabbitMQ on a microk8s cluster using Ansible playbook.
# usage: ./08-setup-rabbitmq.sh [target-host]
# example: ./08-setup-rabbitmq.sh multipass-microk8s
...
PLAYBOOK_PATH_SETUP_RABBITMQ="$ANSIBLE_DIR/playbooks/080-setup-rabbitmq.yml"
...
# Main execution
main() {
    echo "Starting RabbitMQ setup on $TARGET_HOST"
    echo "---------------------------------------------------"

    test_connection || return 1
    
    run_playbook "Setup RabbitMQ" "$PLAYBOOK_PATH_SETUP_RABBITMQ" || return 1
