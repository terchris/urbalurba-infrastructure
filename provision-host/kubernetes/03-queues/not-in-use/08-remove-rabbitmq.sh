#!/bin/bash
# filename: 08-remove-rabbitmq.sh
# description: Remove RabbitMQ from Kubernetes cluster using Ansible playbook
# usage: ./08-remove-rabbitmq.sh [target-host]
# example: ./08-remove-rabbitmq.sh rancher-desktop
...
PLAYBOOK_PATH_REMOVE_RABBITMQ="$ANSIBLE_DIR/playbooks/080-remove-rabbitmq.yml"
...
run_playbook() {
    ...
    echo "🔧 08-remove-rabbitmq.sh: Running Ansible playbook for RabbitMQ removal..."
...
# Main execution
main() {
    echo "🗑️  08-remove-rabbitmq.sh: Removing RabbitMQ from Kubernetes cluster..."
    echo "📍 Target Host: $TARGET_HOST"
    echo "📋 Playbook: $PLAYBOOK_PATH_REMOVE_RABBITMQ"
    echo ""

    test_connection || return 1

    run_playbook "Remove RabbitMQ" "$PLAYBOOK_PATH_REMOVE_RABBITMQ" || return 1
...
print_summary() {
    ...
    if [ ${#ERRORS[@]} -eq 0 ]; then
        echo ""
        echo "✅ 08-remove-rabbitmq.sh: RabbitMQ removal completed successfully!"
        echo "🎯 Target: $TARGET_HOST"
        echo "📝 Note: All RabbitMQ resources have been removed from the cluster"
        echo "🔐 Note: urbalurba-secrets preserved for future deployments"
    else
        echo ""
        echo "❌ Errors occurred during RabbitMQ removal:"
