...
run_playbook() {
    ...
    echo "🔧 06-remove-redis.sh: Running Ansible playbook for Redis removal..."
...
# Main execution
main() {
    echo "🗑️  06-remove-redis.sh: Removing Redis from Kubernetes cluster..."
    echo "📍 Target Host: $TARGET_HOST"
    echo "📋 Playbook: $PLAYBOOK_PATH_REMOVE_REDIS"
    echo ""
...
print_summary() {
    ...
    if [ ${#ERRORS[@]} -eq 0 ]; then
        echo ""
        echo "✅ 06-remove-redis.sh: Redis removal completed successfully!"
        echo "🎯 Target: $TARGET_HOST"
        echo "📝 Note: All Redis resources have been removed from the cluster"
        echo "🔐 Note: urbalurba-secrets preserved for future deployments"
    else
        echo ""
        echo "❌ Errors occurred during Redis removal:"
