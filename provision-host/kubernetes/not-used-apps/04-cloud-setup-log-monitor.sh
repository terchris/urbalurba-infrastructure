#!/bin/bash
# filename: 04-cloud-setup-log-monitor.sh
# description: Setup log monitoring on a microk8s cluster using Ansible playbook. Deploys Prometheus, Grafana, Loki
# usage: ./04-cloud-setup-log-monitor.sh [target-host]
# example: ./04-cloud-setup-log-monitor.sh multipass-microk8s
...
PLAYBOOK_PATH_SETUP_LOGGING="$ANSIBLE_DIR/playbooks/030-logging-monitoring.yml"
...
# Main execution
main() {
    echo "Starting log monitoring setup on $TARGET_HOST"
    echo "---------------------------------------------------"

    test_connection || return 1
    
    run_playbook "Setup Logging and Monitoring" "$PLAYBOOK_PATH_SETUP_LOGGING" || return 1
