...
# File: provision-host/kubernetes/08-development/not-in-use/02-remove-argocd.sh
# Description: Remove ArgoCD deployment from Kubernetes cluster
# Usage: ./02-remove-argocd.sh [target-host]
# Example: ./02-remove-argocd.sh rancher-desktop
#
...
PLAYBOOK_PATH="$ANSIBLE_DIR/playbooks/220-remove-argocd.yml"

# Default target host
TARGET_HOST="${1:-rancher-desktop}"

# Display script purpose
echo "🗑️  $(basename "$0"): Removing ArgoCD from Kubernetes cluster..."
...
run_playbook() {
    echo "🔧 $(basename "$0"): Running Ansible playbook for ArgoCD removal..."
...
if run_playbook; then
    echo
    echo "✅ $(basename "$0"): ArgoCD removal completed successfully!"
    echo "🎯 Target: $TARGET_HOST"
    echo "📝 Note: ArgoCD Helm release and pods have been removed"
    echo "🔐 Note: urbalurba-secrets preserved for future deployments"
    echo "🌐 Note: Traefik IngressRoute configuration has been removed"
    echo "💾 Note: By default, persistent data is preserved"
    echo "    To remove all data: ansible-playbook playbooks/220-remove-argocd.yml -e target_host=$TARGET_HOST -e remove_pvc=true"
else
    echo
    echo "❌ $(basename "$0"): ArgoCD removal failed!"
