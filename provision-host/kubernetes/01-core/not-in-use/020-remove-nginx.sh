...
# File: provision-host/kubernetes/01-core/not-in-use/020-remove-nginx.sh
# Description: Remove Nginx deployment from Kubernetes cluster
# Usage: ./01-remove-nginx.sh [target-host]
# Example: ./01-remove-nginx.sh rancher-desktop
#
...
PLAYBOOK_PATH="$ANSIBLE_DIR/playbooks/020-remove-nginx.yml"

# Default target host
TARGET_HOST="${1:-rancher-desktop}"

# Display script purpose
echo "🗑️  $(basename "$0"): Removing Nginx from Kubernetes cluster..."
...
run_playbook() {
    echo "🔧 $(basename "$0"): Running Ansible playbook for Nginx removal..."
...
if run_playbook; then
    echo
    echo "✅ $(basename "$0"): Nginx removal completed successfully!"
    echo "🎯 Target: $TARGET_HOST"
    echo "📝 Note: All Nginx resources have been removed from the cluster"
else
    echo
    echo "❌ $(basename "$0"): Nginx removal failed!"
