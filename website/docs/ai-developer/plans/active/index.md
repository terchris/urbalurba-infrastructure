...
| [Plan: Move AKS config to `.uis.secrets/cloud-accounts/azure-default.env`](PLAN-aks-config-cloud-accounts.md) | Replace the bash-file-in-tree config (`platforms/azure-aks/azure-aks-config.sh`) with the documented `.uis.secrets/cloud-accounts/azure-default.env` convention. Single user-edited file; defaults visible inline as commented overrides; scripts use `${VAR:-default}` shell fallback. Aligns AKS with the cluster-secret pattern that `secrets.md` already documents. | 2026-05-10 |
