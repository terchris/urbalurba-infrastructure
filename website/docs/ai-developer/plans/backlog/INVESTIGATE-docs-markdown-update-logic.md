...
**Related**:
- [INVESTIGATE-system-service-version-metadata.md](INVESTIGATE-system-service-version-metadata.md) — Version field is also missing from generator
- [INVESTIGATE-backstage.md](../completed/INVESTIGATE-backstage.md) — Adding new metadata fields (`SCRIPT_KIND`, `SCRIPT_TYPE`, `SCRIPT_OWNER`) to all service definitions. The docs generator should consume these too (e.g., display service type badge, ownership info).
