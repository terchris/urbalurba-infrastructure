...
*See [Traefik Ingress Rules](../contributors/rules/ingress-traefik.md) for complete technical details on HostRegexp routing.*
...
*Technical details: [Traefik Ingress Rules Guide](../contributors/rules/ingress-traefik.md)*
...
flowchart TD
    ...
    subgraph Internet_Access ["Internet Access Options"]
        Tailscale_Funnel["Tailscale Funnel service.device.ts.net"]
        CF_Tunnel["Cloudflare Tunnel service.domain.com"]
        Localhost["Localhost service.localhost"]
    end

    subgraph Your_Cluster ["Your Kubernetes Cluster"]
        subgraph Traefik_Layer ["Traefik Layer"]
            Traefik["Traefik 3.3.6 IngressRoute CRDs"]
            HostRegexp["HostRegexp Routing"]
            Internal_DNS["Internal DNS CoreDNS Rewrites"]
        end
...
flowchart TD
    ...
    Tailscale_Client -. "Encrypted P2P" .-> Tailscale_Agent
```
